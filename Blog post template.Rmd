...
```{r}
income_time_limits <- income_time %>%  
  left_join(income_limits, by =c("year")) %>% 
  filter(race != "All Races",
         race != "Asian Alone",
         race != "Asian Alone or in Combination") %>% 
  mutate(ordered_quintile = fct_infreq(income_quintile)) %>% 
  ggplot() +
  geom_boxplot(aes(x = ordered_quintile,
             y = income_dollars,
             fill = race)) +
  #scale_y_continuous(labels=scales::dollar_format()) +
  #scale_fill_manual(values = c("#40E0D0","#6495ED", "#9370DB")) +
...
Sed feugiat lacus nec neque mollis semper. Proin euismod ut diam vitae tristique. Nam varius venenatis dolor, at tristique ligula tincidunt vitae. Nullam cursus sagittis tortor at viverra. Sed vel varius diam, eu consectetur quam. Duis ac blandit lorem. Aenean urna enim, dignissim nec lacinia nec, commodo sit amet ante. Suspendisse fringilla sapien non enim vulputate, et posuere turpis fringilla. Donec vel nunc suscipit, rhoncus nulla vitae, consectetur ex. Fusce ornare sit amet leo ut aliquet. 
